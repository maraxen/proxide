...
fn parse_structure(path: String, spec: Option<OutputSpec>) -> PyResult<PyObject> {
    // Unconditional debug write
    {
        ...
        writeln!(
            f,
            "[parse_structure] path={}, add_hydrogens={}",
            path,
            spec.as_ref().map(|s| s.add_hydrogens).unwrap_or(false)
        )
        .unwrap();
        f.flush().unwrap();
    }
    Python::with_gil(|py| {
        {
            ...
            if let Ok(mut f) = std::fs::OpenOptions::new()
                ...
                .open("/tmp/oxidize_debug.log")
            {
                let _ = writeln!(f, "[parse_structure] Inside with_gil");
                let _ = f.flush();
            }
        }

        eprintln!("[OXIDIZE] Inside with_gil block");
        ...
        if spec.add_hydrogens {
            eprintln!(
                "[OXIDIZE H-ADD STEP 1] Starting hydrogen addition (source: {:?})",
                spec.hydrogen_source
            );
            log::debug!(
                "Adding hydrogens to reference model (source: {:?})...",
                spec.hydrogen_source
            );

            eprintln!("[OXIDIZE H-ADD STEP 2] Extracting coordinates...");
            let all_coords = processed.extract_all_coords();
            eprintln!(
                "[OXIDIZE H-ADD STEP 3] Extracted {} coords",
                all_coords.len()
            );

            let all_elements = &processed.raw_atoms.elements;
            eprintln!("[OXIDIZE H-ADD STEP 4] Got {} elements", all_elements.len());

            // Infer bonds if not provided
            eprintln!(
                "[OXIDIZE H-ADD STEP 5] Inferring bonds for {} atoms...",
                all_coords.len()
            );
            bonds_for_h = geometry::topology::infer_bonds(&all_coords, all_elements, 1.3);
            eprintln!(
                "[OXIDIZE H-ADD STEP 6] Inferred {} bonds",
                bonds_for_h.len()
            );

            eprintln!("[OXIDIZE H-ADD STEP 7] Starting add_hydrogens function...");

            // Add hydrogens based on source selection
            let num_h_added = match spec.hydrogen_source {
                spec::HydrogenSource::FragmentLibrary => {
                    // Use fragment library (Kabsch alignment)
                    geometry::hydrogens::add_hydrogens(&mut processed, &mut bonds_for_h).map_err(
                        |e| {
                            pyo3::exceptions::PyValueError::new_err(format!(
                                "Hydrogen addition (fragment lib) failed: {}",
                                e
                            ))
                        },
                    )?
                }
                spec::HydrogenSource::ForceFieldFirst | spec::HydrogenSource::ForceFieldOnly => {
                    // TODO: Implement FF-based hydrogen generation
                    // For now, fallback to fragment library
                    log::warn!("ForceField-based hydrogen coordinates not yet implemented, using fragment library");
                    ...
                    // File-based debug logging for hang investigation
                    {
                            ...
                            .open("/tmp/oxidize_debug.log")
                        {
                            let _ = writeln!(
                                f,
                                "[lib.rs] Before add_hydrogens call: {} atoms, {} bonds",
                    ...
                    let result =
                        geometry::hydrogens::add_hydrogens(&mut processed, &mut bonds_for_h);

                    {
                            ...
                            .open("/tmp/oxidize_debug.log")
                        {
                            let _ = writeln!(
                                f,
                                "[lib.rs] After add_hydrogens call: result = {:?}",
                                result.is_ok()
                            );
                            let _ = f.flush();
                        }
                    ...
                    result.map_err(|e| {
                        pyo3::exceptions::PyValueError::new_err(format!(
                            "Hydrogen addition (FF fallback) failed: {}",
                            e
                        ))
                    })?
                }
            ...
            // Run energy minimization if requested
            if spec.relax_hydrogens && num_h_added > 0 {
                let max_iter = spec.relax_max_iterations.unwrap_or(100);
                log::debug!(
                    "Relaxing hydrogen positions (max {} iterations)...",
                    max_iter
                );

                // Extract coords for relaxation
                let n_atoms = processed.raw_atoms.num_atoms;
                let mut coords: Vec<[f32; 3]> = (0..n_atoms)
                    .map(|i| {
                        [
                            processed.raw_atoms.coords[i * 3],
                            processed.raw_atoms.coords[i * 3 + 1],
                            processed.raw_atoms.coords[i * 3 + 2],
                        ]
                    })
                    .collect();

                let elements = processed.raw_atoms.elements.clone();
                let options = geometry::relax::RelaxOptions {
                    max_iterations: Some(max_iter),
                    angle_increment: 10.0_f32.to_radians(),
                    force_cutoff: 10.0,
                };

                let (iterations, final_energy) = geometry::relax::relax_hydrogens(
                    &mut coords,
                    &elements,
                    None, // charges optional
                    &bonds_for_h,
                    &options,
                );

                // Update structure with relaxed coords
                for i in 0..n_atoms {
                    processed.raw_atoms.coords[i * 3] = coords[i][0];
                    processed.raw_atoms.coords[i * 3 + 1] = coords[i][1];
                    processed.raw_atoms.coords[i * 3 + 2] = coords[i][2];
                }

                log::debug!(
                    "Relaxed in {} iterations (final energy: {:.2})",
                    iterations,
                    final_energy
                );
            }
