...
impl Atom37Formatter {
    ...
    pub fn format(processed: &ProcessedStructure, _spec: &OutputSpec) -> Result<FormattedAtom37, String> {
...
mod tests {
    ...
    fn test_atom37_single_residue_ala() {
        ...
        // Verify dimensions
        assert_eq!(formatted.coordinates.len(), 1 * 37 * 3);
        assert_eq!(formatted.atom_mask.len(), 1 * 37);
        ...
        for atom_name in &["N", "CA", "C", "CB", "O"] {
            ...
            assert_eq!(formatted.atom_mask[idx], 1.0, "Atom {} should be present", atom_name);
        }
