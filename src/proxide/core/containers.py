...
class Protein:
  ...
  @property
  def atom_residue_ids(self) -> jnp.ndarray | np.ndarray:
  ...
  def from_rust_dict(
    ...
    a `Protein` dataclass, handling type conversion (JAX/NumPy) and reshaping for Atom37.
    ...
    Process:
        ...
        3.  **Units**: Data arrives pre-converted from Rust based on ``OutputSpec.unit_system``.
            Default is Amber (Å + kcal/mol). No unit conversion is performed in Python.
    ...
    def _slice_if_padded(val, dtype=None):
      if val is None:
        return None
    ...
    return cls(
      coordinates=_slice_if_padded(raw_coords.reshape(-1, 3)),
      full_coordinates=_slice_if_padded(raw_coords.reshape(-1, 3)),
      aatype=convert(rust_dict["aatype"], dtype=np.int8),
      one_hot_sequence=convert(np.eye(21)[rust_dict["aatype"]]),
      mask=convert(np.ones(num_residues), dtype=np.float32),
      residue_index=_slice_if_padded(rust_dict["residue_index"], dtype=np.int32),
      chain_index=_slice_if_padded(rust_dict["chain_index"], dtype=np.int32),
      ...
      format="Full",
      source=source,
      masses=_slice_if_padded(rust_dict.get("masses")),
      constrained_bonds=convert(rust_dict.get("constrained_bonds"))
      if rust_dict.get("constrained_bonds") is not None
      else None,
      constrained_bond_lengths=convert(
