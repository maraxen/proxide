...
if TYPE_CHECKING:
    pass  # RDKit types would go here
...
class Molecule:
    ...
    def from_sdf(cls, path: str | Path, conformer_idx: int = 0) -> "Molecule":
        ...
        if conformer_idx >= len(molecules):
            raise ValueError(f"Conformer index {conformer_idx} out of range (file has {len(molecules)} molecules)")
    ...
    def assign_charges_openff(self, method: str = "am1bcc") -> None:
        """Assign partial charges using OpenFF toolkit.
        
        Requires openff-toolkit and RDKit.
        
        Args:
            method: Charge method to use. Options: "am1bcc", "am1-mulliken".
                   Default is "am1bcc" (AM1-BCC).
        """
        try:
            from openff.toolkit import Molecule as OFFMolecule
        except ImportError as e:
            raise ImportError(
                "OpenFF toolkit is required for charge assignment. "
                "Install with: pip install openff-toolkit"
            ) from e
        
        # Create OpenFF Molecule from our data
        off_mol = OFFMolecule.from_rdkit(self._to_rdkit())
        ...
        self.charges = np.array(
            [charge.magnitude for charge in off_mol.partial_charges],
            dtype=np.float32
        )
    ...
    def _to_rdkit(self):
        ...
        try:
            ...
            raise ImportError(
                "RDKit is required for this operation. "
                "Install with: pip install rdkit"
            ) from e
