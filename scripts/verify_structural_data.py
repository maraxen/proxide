...
def verify_structural_data():
  ...
  if protein.proper_dihedrals is not None:
    ...
    # Note: 1crn is small, might not have many dihedrals if logic is strict,
    # but usually should have some.
    # But wait, does 1crn (crambin) have proper dihedrals? Yes.
    print(
      "WARNING: proper_dihedrals is None (might be expected for simple inference "
      "if no ForceField XML providing params, but GAFF typing implies inference?)"
    )
    ...
    # if spec.infer_bonds {
    #   let topology = ...;
    #   if !topology.proper_dihedrals.is_empty() { dict.set_item("dihedrals", ...) }
    # }
