...
use common::{make_synthetic_backbone, make_synthetic_backbone_partial_atoms, make_synthetic_backbone_no_atoms};
...
fn bb_atoms_count_full_residue() {
    ...
    for ri in &atom_res {
        assert_eq!(*ri, ResidueIndex(0), "All atoms should belong to ResidueIndex(0)");
    }
}
...
fn bb_atoms_two_residues_ordering() {
    ...
    assert_eq!(atoms.len(), 8, "Two full residues should emit 8 atoms total");
...
fn bb_atoms_all_none_atoms() {
    ...
    assert_eq!(atoms.len(), 0, "Residue with all None atoms should emit no atoms");
