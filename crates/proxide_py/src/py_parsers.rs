...
pub fn parse_structure(
    ...
    if spec.add_hydrogens {
        ...
        if num_h_added > 0 {
            ...
            let (rebuilt, rebuilt_bonds) =
                crate::processing::residues::rebuild_topology(processed, bonds_for_h).map_err(
                    |e| {
    ...
    if spec.parameterize_md {
        if let Some(ref ff_path) = spec.force_field {
            ...
            let ff = py
                .allow_threads(|| forcefield::parse_forcefield_xml(ff_path))
                .map_err(|e| {
                    pyo3::exceptions::PyValueError::new_err(format!(
            ...
            let param_options = physics::md_params::ParamOptions {
                ...
                missing_mode: match spec.missing_residue_mode {
                    ...
                    spec::MissingResidueMode::Fail => physics::md_params::MissingResidueMode::Fail,
    ...
    if spec.compute_rbf {
        ...
        let neighbors_reshaped = neighbors_array.reshape((n_res, k_neighbors)).map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!(
        ...
        let rbf_array = flat_array.reshape(shape).map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!("Failed to reshape RBF array: {}", e))
        })?;
    ...
    if spec.infer_bonds {
        ...
        if !topology.bonds.is_empty() {
            ...
            let bonds_reshaped = bonds_array
    ...
    if spec.compute_vdw {
        ...
        let flat_array = PyArray1::from_slice_bound(py, &features);
        let array = flat_array.reshape(shape).map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!("Failed to reshape VdW array: {}", e))
        })?;
...
pub fn project_to_mpnn_batch(
    ...
    .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("Projection failed: {}", e)))?;
...
impl FoldCompDatabase {
    ...
    pub fn get_by_name(&self, name: String) -> PyResult<PyAtomicSystem> {
        self.inner
            .get_by_name(&name)
            .map(PyAtomicSystem::from_core)
            .map_err(|e| {
