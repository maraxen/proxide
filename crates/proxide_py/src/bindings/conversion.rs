...
impl ToPyDict for RawAtomData {
    fn to_py_dict<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyDict>> {
        ...
        let reshaped = coords_arr.reshape((self.num_atoms, 3)).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Failed to reshape coords: {}", e))
        })?;
...
impl ToPyDict for CachedStructure {
    fn to_py_dict<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyDict>> {
        let dict = PyDict::new_bound(py);
        dict.set_item("coordinates", PyArray1::from_slice_bound(py, &self.coordinates))?;
        ...
        dict.set_item("residue_index", PyArray1::from_slice_bound(py, &self.residue_index))?;
        dict.set_item("chain_index", PyArray1::from_slice_bound(py, &self.chain_index))?;
