...
pub fn parse_mdtraj_h5_metadata(py: Python<'_>, path: String) -> PyResult<PyObject> {
    let result = py
        .allow_threads(|| formats::mdtraj_h5::parse_mdtraj_h5_metadata(&path))
...
pub fn parse_mdtraj_h5_frame(py: Python<'_>, path: String, frame_idx: usize) -> PyResult<PyObject> {
    let frame = py
        .allow_threads(|| formats::mdtraj_h5::parse_mdtraj_h5_frame(&path, frame_idx))
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
...
pub fn parse_mdcath_metadata(py: Python<'_>, path: String) -> PyResult<PyObject> {
    let result = py
        .allow_threads(|| formats::mdcath_h5::parse_mdcath_metadata(&path))
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
...
pub fn parse_mdcath_frame(
    ...
    let frame = py
        .allow_threads(|| {
            formats::mdcath_h5::parse_mdcath_frame(
                &path,
                &domain_id,
                &temperature,
                &replica,
