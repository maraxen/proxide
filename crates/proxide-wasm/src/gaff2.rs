...
pub fn assign_params(mol: &WasmMol) -> ParamSet {
    ...
    let elements: Vec<String> = mol.atoms.iter()
    ...
    let topo_bonds: Vec<TopoBond> = mol.bonds.iter()
        .map(|b| TopoBond::new(b.i, b.j))
        .collect();
    ...
    for (idx, atom) in mol.atoms.iter().enumerate() {
        if atom.is_aromatic && atom.atomic_num == 6 && atom_types.get(idx).map(|t| t.starts_with('c')).unwrap_or(false) {
        ...
        if atom.is_aromatic && atom.atomic_num == 7 && atom_types.get(idx).map(|t| t.starts_with('n')).unwrap_or(false) {
    ...
    let atoms: Vec<AtomRecord> = mol.atoms.iter().enumerate().map(|(idx, a)| AtomRecord {
        idx,
    ...
    let bonds: Vec<BondRecord> = topology.bonds.iter().filter_map(|b| {
    ...
    let angles: Vec<AngleRecord> = topology.angles.iter().filter_map(|a| {
...
mod tests {
    ...
    fn methane_atom_types() {
        ...
        assert_eq!(params.atoms[0].atom_type, "c3", "methane carbon should be c3");
    }
    ...
    fn benzene_carbons_are_ca_type() {
        ...
        let params = assign_params(&mol);
        assert!(params.atoms.iter().all(|a| a.atom_type == "ca"),
    ...
    fn ethanol_has_correct_atom_types() {
        ...
        assert!(params.atoms.iter().any(|a| a.atom_type.starts_with('o')),
