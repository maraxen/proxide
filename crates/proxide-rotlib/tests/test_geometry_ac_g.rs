...
fn test_ac_g_a_chi_recovery_endo() {
    ...
    println!(
        "AC-G(a) endo: χ1_in={:.1}°, χ1_out={:.1}°, err={:.1}°",
        ENDO_CHI[0], result.recovered_chi[0], chi1_err
    );
    println!(
        "AC-G(a) endo: χ2_in={:.1}°, χ2_out={:.1}°, err={:.1}°",
        ENDO_CHI[1], result.recovered_chi[1], chi2_err
    );
    println!(
        "AC-G(a) endo: χ3_out={:.1}° (ring closure output)",
        result.recovered_chi[2]
    );
...
fn test_ac_g_a_chi_recovery_exo() {
    ...
    println!(
        "AC-G(a) exo: χ1_in={:.1}°, χ1_out={:.1}°, err={:.1}°",
        EXO_CHI[0], result.recovered_chi[0], chi1_err
    );
    println!(
        "AC-G(a) exo: χ2_in={:.1}°, χ2_out={:.1}°, err={:.1}°",
        EXO_CHI[1], result.recovered_chi[1], chi2_err
    );
    println!(
        "AC-G(a) exo: χ3_out={:.1}° (ring closure output)",
        result.recovered_chi[2]
    );
...
fn test_ac_g_b_pucker_distinction() {
    let (builder, bb_frame) = make_builder_and_frame();
    let endo = builder
        .build(&bb_frame, ENDO_CHI)
        .expect("endo build failed");
    ...
    println!(
        "AC-G(b): CG_endo=({:.3},{:.3},{:.3})",
        cg_endo[0], cg_endo[1], cg_endo[2]
    );
    println!(
        "AC-G(b): CG_exo =({:.3},{:.3},{:.3})",
        cg_exo[0], cg_exo[1], cg_exo[2]
    );
...
fn test_ac_g_c_ring_angles() {
    ...
    assert!(
        (ang_ca_cb_cg - ccd_ideals::CA_CB_CG).abs() <= 3.0,
        "CA-CB-CG"
    );
    ...
    assert!(
        (ang_cb_cg_cd - ccd_ideals::CB_CG_CD).abs() <= 6.0,
        "solved CB-CG-CD {:.1}° outside ±6° of ideal {:.1}°",
        ang_cb_cg_cd,
        ccd_ideals::CB_CG_CD
    );
    assert!(
        (ang_cg_cd_n - ccd_ideals::CG_CD_N).abs() <= 6.0,
        "emergent CG-CD-N {:.1}° outside ±6° of ideal {:.1}°",
        ang_cg_cd_n,
        ccd_ideals::CG_CD_N
    );
    assert!(
        (ang_cd_n_ca - ccd_ideals::CD_N_CA).abs() <= 6.0,
        "emergent CD-N-CA {:.1}° outside ±6° of ideal {:.1}°",
        ang_cd_n_ca,
        ccd_ideals::CD_N_CA
    );
}
...
fn test_ac_g_e_round_trip_identity() {
    ...
    let built = builder
        .build(&bb_identity_f32, ENDO_CHI)
    ...
    assert_eq!(
        built.sidechain.len(),
        3,
        "ProlineBuilder must return 3 atoms (CB, CG, CD)"
    );
    let [cb_built, cg_built, cd_built] =
        [built.sidechain[0], built.sidechain[1], built.sidechain[2]];
    ...
    let placed = lib
        .place_rotamer("CPR", -180.0, -180.0, 0, false, n_f64, ca_f64, c_f64)
