...
use proxide_rotlib::pb::rotlib_v1::{
    Bin, ChiValue, GeometryMode, IcRecord, ResidueEntry, ResidueGeometry, ResidueGeometryTable,
    Rotamer, RotamerLibrary, Vec3,
};
...
fn test_roundtrip_with_zstd() {
    ...
    // Compress
    let compressed = zstd::encode_all(prost_bytes.as_slice(), 3).expect("zstd compress failed");
    assert!(
        compressed.len() < prost_bytes.len() + 100,
        "compressed should not be vastly larger"
    );
    // Decompress
    let decompressed = zstd::decode_all(compressed.as_slice()).expect("zstd decompress failed");
    assert_eq!(
        decompressed, prost_bytes,
        "zstd round-trip must be byte-identical"
    );
    // Decode protobuf
    let decoded =
        RotamerLibrary::decode(decompressed.as_slice()).expect("prost decode after zstd failed");
...
fn test_attribution_field_present() {
    ...
    assert!(
        !decoded.attribution.is_empty(),
        "attribution must survive round-trip non-empty"
    );
}
...
fn test_residue_geometry_table_roundtrip_prost() {
    ...
    assert!(
        !bytes.is_empty(),
        "encoded geometry table must be non-empty"
    );
    ...
    assert!(
        (ic.b_ij - 1.5460).abs() < 1e-5,
        "bond length i-j round-trip failed"
    );
    assert!(
        (ic.theta_ijk - 110.2800).abs() < 1e-4,
        "angle i-j-k round-trip failed"
    );
    assert!(
        (ic.phi_ijkl - 180.0).abs() < 1e-4,
        "dihedral round-trip failed"
    );
    assert!(
        (ic.theta_jkl - 98.9400).abs() < 1e-4,
        "angle j-k-l round-trip failed"
    );
    assert!(
        (ic.b_kl - 1.8206).abs() < 1e-5,
        "bond length k-l round-trip failed"
    );
}
...
fn test_rotamer_library_with_geometry_fields() {
    ...
    let bytes = lib.encode_to_vec();
    let decoded =
        RotamerLibrary::decode(bytes.as_slice()).expect("prost decode of RotamerLibrary failed");

    assert_eq!(decoded.geometry_source, "charmm36");
    assert_eq!(
        decoded.geometry_license,
        "NOT-OSI: MacKerell lab academic use only"
    );
    assert_eq!(decoded.data_license, "ODC-BY-1.0");
}
