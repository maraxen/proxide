...
pub fn parse_rtf_ic_table(rtf_path: &str) -> Result<ResidueGeometryTable, RotlibError> {
    ...
    for line in reader.lines() {
        let line =
            line.map_err(|e| RotlibError::InvalidFormat(format!("IO error reading RTF: {}", e)))?;
...
fn parse_ic_line(line: &str) -> Result<IcRecord, RotlibError> {
    ...
    if parts.len() < 10 {
        return Err(RotlibError::InvalidFormat(format!(
            "IC line has {} fields, need at least 10",
            parts.len()
        )));
    }
...
mod tests {
    ...
    const RTF_PATH: &str =
        "/home/marielle/.local/share/mamba/envs/espaloma-bench/dat/chamber/top_all36_prot.rtf";
    ...
    fn test_parse_rtf_ic_table_real_file() {
        ...
        assert!(
            table.residues.len() >= 20,
            "Expected at least 20 residues, got {}",
            table.residues.len()
        );
    }
    ...
    fn test_met_ce_bond() {
        ...
        let met = table
            ...
            .iter()
            .find(|r| r.name == "MET")
            .expect("MET not found");
        let ce_rec = met.ic.iter().find(|ic| {
            ic.atom_i == "CB" && ic.atom_j == "CG" && ic.atom_k == "SD" && ic.atom_l == "CE"
        });
        assert!(ce_rec.is_some(), "CB-CG-SD-CE IC record not found");
        let ic = ce_rec.unwrap();
        assert!(
            (ic.b_kl - 1.8206).abs() < 0.001,
            "CE bond: expected ~1.8206 Å, got {}",
            ic.b_kl
        );
        assert!(
            (ic.theta_jkl - 98.94).abs() < 0.1,
            "CE angle: expected ~98.94°, got {}",
            ic.theta_jkl
        );
    }
    ...
    fn test_all_20_standard_aa_present() {
        ...
        let table = parse_rtf_ic_table(RTF_PATH).expect("Parse RTF");
        for aa in &[
            "ALA", "ARG", "ASN", "ASP", "CYS", "GLN", "GLU", "GLY", "HSD", "ILE", "LEU", "LYS",
            "MET", "PHE", "PRO", "SER", "THR", "TRP", "TYR", "VAL",
        ] {
            assert!(
                table.residues.iter().any(|r| r.name == *aa),
                "{} not found in RTF table",
                aa
            );
        }
    }
