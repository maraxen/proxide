    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for size in sizes:
            ...
            if BIOTITE_AVAILABLE:
                # Warmup
                try:
                    res = list(load_biotite(str(pdb_file)))
                except Exception as e:
            ...
            print(f"PDB Parsing ({size} res)           | {rust_time:.5f}    | {py_time:.5f}    | {speedup:.1f}x")
        ...
        for size in sizes:
            ...
            if BIOTITE_AVAILABLE:
                ...
                with BenchmarkTimer("Py CIF") as t_py:
                    for _ in range(5):
                        try:
                            items = list(load_biotite(str(cif_file)))
                        except Exception:
            ...
            speedup = py_time / rust_time if rust_time > 0 and py_time > 0 else 0.0
            print(f"mmCIF Parsing ({size} res)         | {rust_time:.5f}    | {py_time:.5f}    | {speedup:.1f}x")
...
"""
        ...
        speedup = py_time / rust_time if rust_time > 0 and py_time > 0 else 0.0
        print(f"Force Field Loading (100 types)    | {rust_time:.5f}    | {py_time:.5f}    | {speedup:.1f}x")
