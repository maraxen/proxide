...
use crate::physics::constants::{MIN_DISTANCE, MAX_FORCE, DEFAULT_SIGMA, DEFAULT_EPSILON};
...
/// ε_ij = sqrt(ε_i * ε_j)
#[inline]
pub fn combine_lj_parameters(sigma_i: f32, sigma_j: f32, epsilon_i: f32, epsilon_j: f32) -> (f32, f32) {
...
pub fn compute_lj_forces(
    ...
    for i in 0..n {
        ...
        for j in 0..m {
            ...
            let (sigma_ij, epsilon_ij) = combine_lj_parameters(
                target_sigmas[i], source_sigmas[j],
                target_epsilons[i], source_epsilons[j],
            );
    ...
    for i in 0..n_res {
        for j in 0..5 {
            ...
            for (k, source) in all_positions.iter().enumerate() {
                ...
                let (sigma_ij, epsilon_ij) = combine_lj_parameters(
                    DEFAULT_SIGMA, all_sigmas[k],
                    DEFAULT_EPSILON, all_epsilons[k],
                );
...
mod tests {
    ...
    fn test_lj_force_sign() {
        ...
        assert!(force_close > 0.0, "Force should be repulsive at short distance");
        
        // At distance = equilibrium, force should be ~0
        let r_min = sigma * 2.0f32.powf(1.0 / 6.0);
        let force_eq = compute_lj_force_magnitude(r_min, sigma, epsilon);
        assert!(force_eq.abs() < 0.01, "Force should be near zero at equilibrium");
        ...
        assert!(force_far < 0.0, "Force should be attractive at medium distance");
    }
    ...
    fn test_lj_forces_repulsion() {
        ...
        let forces = compute_lj_forces(&target, &source, &sigmas, &epsilons, &sigmas, &epsilons, false);
